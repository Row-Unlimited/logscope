...
def COMMAND(input_dict):
    """Creates a COMMAND event"""
    input_dict["OBJ_TYPE"] = "COMMAND"
    return input_dict


def EVR(input_dict):
    """Creates an EVR event"""
    input_dict["OBJ_TYPE"] = "EVR"
    return input_dict


def CHANNEL(input_dict):
    """Creates a CHANNEL event"""
    input_dict["OBJ_TYPE"] = "CHANNEL"
    return input_dict


def CHANGE(input_dict):
    """Creates a CHANGE event"""
    input_dict["OBJ_TYPE"] = "CHANGE"
    return input_dict


def PRODUCT(input_dict):
    """Creates a PRODUCT event"""
    input_dict["OBJ_TYPE"] = "PRODUCT"
    return input_dict
...
def checkErrorCounts_(results, resultmap):
    ...
    @param @c resultmap : map[string,int], expected error counts for each property
    @result @c void
    """
    map = resultmap.copy()
    for result in results:
...
def checkErrors_(results, errorSpecs):
    ...
    for result in results:
        errors += result.getErrors()
    assert len(errors) == len(errorSpecs), "errors detected: " + str(len(errors)) + ", errors expected: " + str(
        len(errorSpecs))
...
def rewriteErrorSpec_(errorSpec):
    return dict([(rewriteErrorKey_(key), value) for key, value in errorSpec.items()])
...
def wfErrorSpec_(errorSpec):
    ...
    assert set(errorSpec.keys()).issubset(list(specfields.keys())), str(list(errorSpec.keys())) + str(
        list(specfields.keys()))
    for field, value in errorSpec.items():
...
def checkBasics_(error, errorSpec):
    ...
    if state is None:
        assert {"state", "bindings", "history", "historylength"}.intersection(list(errorSpec.keys())) == set([]), str(errorSpec)
    else:
        ...
        if "bindings" in errorSpec:
            ...
            for field, value in errorSpec["bindings"].items():
                assert field in bindings and bindings[field] == value, field + " not in " + str(
                    bindings) + " or =/=" + str(value)
        historychain = state.getHistory()
        if historychain is None:
            history = []
        else:
        ...
        if "history" in errorSpec:
            ...
            for index in range(len(historySpec)):
                ...
                event = history[index]
                for field, value in eventSpec.items():
...
def checkSafetyError_(error, errorSpec):
    ...
    if "event" in errorSpec:
        ...
        for field, value in errorSpec["event"].items():
